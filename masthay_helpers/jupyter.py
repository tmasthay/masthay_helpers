...
import panel as pn
import torch
from .global_helpers import filter_kw
...
def stacked_iplot_workhorse(data_frame, cols=1, **kw):
    # (1) Check if DataFrame has at least 3 columns
    if len(data_frame.columns) < 3:
        raise ValueError(
            "The DataFrame should have at least 3 columns. Use a different"
    ...
    def plot_1D(*indices):
        kw["xlabel"] = index_names[get_axes(indices)[0]]
        lcl_kw = filter_kw(hv.Curve.opts, kw)
        return hv.Curve(data[tuple(indices)]).opts(lcl_kw)

    def plot_2D(*indices, kdims):
        kw["ylabel"] = index_names[get_axes(indices)[1]]
        kw["xlabel"] = index_names[get_axes(indices)[0]]
        lcl_kw = filter_kw(hv.Image.opts, kw)
        plots = [
            hv.Image(data[i, tuple(indices)], kdims=kdims).opts(**lcl_kw)
